const ArgumentType = require('../../extension-support/argument-type');
const BlockType = require('../../extension-support/block-type');
const Cast = require('../../util/cast');
const log = require('../../util/log');
const formatMessage = require('format-message');  //多言語化のために必要

//ブロックに付けるアイコン
const blockIconURI = 'data:image/svg+xml;base64,'

//メニューに付けるアイコン
const menuIconURI = 'data:image/svg+xml;base64,'
//メニュー定義
const UnifiedAPIMenus = {
    menuGPIOdirection: {
        items: [
            { text: 'GPIO::OUT', value: 'GPIO::OUT' },
            { text: 'GPIO::IN',  value: 'GPIO::IN' },
	    { text: 'GPIO::IN|GPIO::PULL_UP',   value: 'GPIO::IN|GPIO::PULL_UP' },
	    { text: 'GPIO::IN|GPIO::PULL_DOWN', value: 'GPIO::IN|GPIO::PULL_DOWN' }
	]
    },
    menuUARTbuffer: {
        items: [
            { text: 'TX', value: 'clear_tx_buffer' },
            { text: 'RX', value: 'clear_rx_buffer' }
	]
    },
    menuTools: {
        items: [
	    { id: 'unifiedapi.menutools.str',    default: '.to_s',     value: 'to_s' },
            { id: 'unifiedapi.menutools.str16',  default: '.to_s(16)', value: 'to_s(16)' },
	    { id: 'unifiedapi.menutools.ord',    default: '.ord',      value: 'ord' },
            { id: 'unifiedapi.menutools.bytes',  default: '.bytes',    value: 'bytes' },
	    { id: 'unifiedapi.menutools.split',  default: '.split',    value: 'split(",")' },
	    { id: 'unifiedapi.menutools.size',   default: '.size',     value: 'size' }
	]
    }
};

function createMenuItems(menuKey) {
    return UnifiedAPIMenus[menuKey].items.map(item => ({
        text: item.text || formatMessage({ id: item.id, default: item.default }),
        value: item.value
    }));
}

//クラス定義
class UnifiedAPI {
    constructor (runtime) {
        this.runtime = runtime;
    }

    getInfo () {
        return {
            id: 'unifiedapi',
            name:formatMessage({ id: 'unifiedapi.name', default: 'Unifiedapi' }),
            menuIconURI: menuIconURI,
            blockIconURI: blockIconURI,
	    color1: '#4CBFE6',
	    color2: '#45AABF',
	    color3: '#2E8EB8',
	    blocks: [
                {
                    opcode :'gpio_init',
                    text: formatMessage({
                        id: 'unifiedapi.gpio_init',
                        default:'GPIO: setup pin [PIN] named [INSTANCE] (mode: [DIRECTION])',
                    }),
                    blockType:BlockType.COMMAND,
                    arguments: {
                        PIN: {
                            type: ArgumentType.NUMBER,
                            defaultValue: 13
                        },
                        DIRECTION: {
                            type: ArgumentType.STRING,
                            menu: 'menuGPIOdirection'
                        },
			INSTANCE:{
			    type: ArgumentType.STRING,
			    defaultValue: '$gpio'
			}			
                    }
                },
                {
                    opcode :'gpio_write',
                    text: formatMessage({
			id: 'unifiedapi.gpio_write',
                        default:'GPIO: write [VALUE] to [INSTANCE]',
                    }),
                    blockType:BlockType.COMMAND,
                    arguments: {
                        VALUE: {
                            type: ArgumentType.NUMBER,
                            defaultValue: 1
                        },
			INSTANCE:{
			    type: ArgumentType.STRING,
			    defaultValue: '$gpio'
			}
                    }
                },
                {                    
                    opcode :'gpio_read',
                    text: formatMessage({
			id: 'unifiedapi.gpio_read',
                        default:'GPIO: [INSTANCE] read value',
                    }),
                    blockType:BlockType.REPORTER,
                    arguments: {
			INSTANCE:{
			    type: ArgumentType.STRING,
			    defaultValue: '$gpio'
			}
                    }
                },
                {                    
                    opcode :'pwm_init',
                    text: formatMessage({
                        id: 'unifiedapi.pwm_init',
                        default:'PWM: setup pin [PIN] named [INSTANCE] (timer: [TIMER], frequency: [FREQ] Hz, duty: [DUTY]%)',
                    }),
                    blockType:BlockType.COMMAND,
                    arguments: {
                        PIN: {
                            type: ArgumentType.NUMBER,
                            defaultValue: 15
                        },
                        TIMER: {
                            type: ArgumentType.NUMBER,
                            defaultValue: 0
                        },
                        FREQ: {
                            type: ArgumentType.NUMBER,
                            defaultValue: 1000
                        },
                        DUTY: {
                            type: ArgumentType.NUMBER,
                            defaultValue: 0
                        },
			INSTANCE:{
			    type: ArgumentType.STRING,
			    defaultValue: '$pwm'
			}			
                    }
                },
                {                    
                    opcode :'pwm_duty',
                    text: formatMessage({
                        id: 'unifiedapi.pwm_duty',
                        default:'PWM: set [INSTANCE] duty to [DUTY] %',
                    }),
                    blockType:BlockType.COMMAND,
                    arguments: {
                        INSTANCE: {
                            type: ArgumentType.STRING,
                            defaultValue: '$pwm'
                        },                        
                        DUTY: {
                            type: ArgumentType.NUMBER,
                            defaultValue: 100
                        },
                        
                    }
                },
                {
                    opcode :'pwm_frequency',
                    text: formatMessage({
                        id: 'unifiedapi.pwm_frequency',
                        default:'PWM: set [INSTANCE] frequency to [FREQ] Hz',
                    }),
                    blockType:BlockType.COMMAND,
                    arguments: {
                        INSTANCE: {
                            type: ArgumentType.STRING,
                            defaultValue: '$pwm'
                        },                        
                        FREQ: {
                            type: ArgumentType.NUMBER,
                            defaultValue: 440
                        },
                        
                    }
                },
                {
                    opcode :'pwm_pulse',
                    text: formatMessage({
                        id: 'unifiedapi.pwm_pulse',
                        default:'PWM: set [INSTANCE] pulse to [PULSE] us',
                    }),
                    blockType:BlockType.COMMAND,
                    arguments: {
                        INSTANCE: {
                            type: ArgumentType.STRING,
                            defaultValue: '$pwm'
                        },                        
                        PULSE: {
                            type: ArgumentType.NUMBER,
                            defaultValue: 1000
                        },
                        
                    }
                },
                {                    
                    opcode :'adc_init',
                    text: formatMessage({
                        id: 'unifiedapi.adc_init',
                        default:'ADC: setup pin [PIN] named [INSTANCE]',
                    }),
                    blockType:BlockType.COMMAND,
                    arguments: {
                        PIN: {
                            type: ArgumentType.NUMBER,
                            defaultValue: 39
                        },
			INSTANCE:{
			    type: ArgumentType.STRING,
			    defaultValue: '$adc'
			}
                    }
                },
                {
                    opcode :'adc_raw',
                    text: formatMessage({
                        id: 'unifiedapi.adc_raw',
                        default:'ADC: [INSTANCE] read raw count',
                    }),
                    blockType:BlockType.REPORTER,
                    arguments: {                     
                        INSTANCE: {
			    type: ArgumentType.STRING,    
                            defaultValue: '$adc'
                        },
                    }
                },
                {
                    opcode :'adc_volt',
                    text: formatMessage({
                        id: 'unifiedapi.adc_volt',
                        default:'ADC: [INSTANCE] read voltage',
                    }),
                    blockType:BlockType.REPORTER,
                    arguments: {                     
                        INSTANCE: {
                            type: ArgumentType.STRING,
                            defaultValue: '$adc'
                        },
                    }
                },
                {
                    opcode: 'i2c_init',
                    text: formatMessage({
                        id: 'unifiedapi.i2c_init',
                        default: 'I2C: setup named [INSTANCE]' 
                    }),
                    blockType: BlockType.COMMAND,
                    arguments: {                     
                        INSTANCE: {
                            type: ArgumentType.STRING,
                            defaultValue: '$i2c'
                        },
                    }
                },
                {
                    opcode: 'i2c_write',
                    text: formatMessage({
                        id: 'unifiedapi.i2c_write',
                        default: 'I2C [ADDR]: write [INSTANCE] to [HEX1], [HEX2]'
                    }),		    
                    blockType: BlockType.COMMAND,
                    arguments: {
                        INSTANCE: {
			    type: ArgumentType.STRING,
                            defaultValue: '$i2c'
			},
			ADDR: {
                            type: ArgumentType.STRING, 
                            defaultValue: '0x00'
                        },
                        HEX1: {
                            type: ArgumentType.STRING, 
                            defaultValue: '0x00'
                        },
                        HEX2: {
                            type: ArgumentType.STRING, 
                            defaultValue: '-'
                        }
                    }
                },
                {
                    opcode: 'i2c_read',
                    text: formatMessage({
                        id: 'unifiedapi.i2c_read',
                        default: 'I2C [ADDR]: [INSTANCE] read [BYTES] bytes from [HEX]'
                    }),	    
                    blockType: BlockType.REPORTER,
                    arguments: {
                        INSTANCE: {
                            type: ArgumentType.STRING,
                            defaultValue: '$i2c'
			},
			ADDR: {
                            type: ArgumentType.STRING, 
                            defaultValue: '0x00'
                        },
			BYTES: {
                            type: ArgumentType.NUMBER,
                            defaultValue: 1
                        },
                        HEX: {
                            type: ArgumentType.STRING, 
                            defaultValue: '-'                            
                        }
                    }
                },
                {
                    opcode: 'spi_init',
                    text: formatMessage({
                        id: 'unifiedapi.spi_init',
                        default: 'SPI: setup named [INSTANCE] (MISO: [MISO], MOSI: [MOSI], CLK: [CLK])' 
                    }),
                    blockType: BlockType.COMMAND,
                    arguments: {
                        INSTANCE: {
                            type: ArgumentType.STRING,
                            defaultValue: '$spi'
			},
                        MISO: {
                            type: ArgumentType.NUMBER,
                            defaultValue: 19
                        },
                        MOSI: {
                            type: ArgumentType.NUMBER,
                            defaultValue: 23
                        },
			CLK: {
                            type: ArgumentType.NUMBER,
                            defaultValue: 18
                        }
                    }
                },
                {
                    opcode: 'spi_write',
                    text: formatMessage({
                        id: 'unifiedapi.spi_write',
                        default: 'SPI: write [INSTANCE] to [HEX1], [HEX2]'
                    }),		    
                    blockType: BlockType.COMMAND,
                    arguments: {
                        INSTANCE: {
                            type: ArgumentType.STRING,
                            defaultValue: '$spi'
			},
                        HEX1: {
                            type: ArgumentType.STRING, 
                            defaultValue: '0x00'
                        },
                        HEX2: {
                            type: ArgumentType.STRING, 
                            defaultValue: '0x00'
                        }
                    }
                },
                {
                    opcode: 'spi_read',
                    text: formatMessage({
                        id: 'unifiedapi.spi_read',
                        default: 'SPI: [INSTANCE] read [BYTES] bytes'
                    }),		    		    
                    blockType: BlockType.REPORTER,
                    arguments: {
                        INSTANCE: {
                            type: ArgumentType.STRING,
                            defaultValue: '$spi'
			},			
			BYTES: {
                            type: ArgumentType.NUMBER,
                            defaultValue: 1
                        }
                    }
                },
                {
                    opcode: 'uart_init',
                    text: formatMessage({
                        id: 'unifiedapi.uart_init',
                        default: 'UART: setup ID [UART] named [INSTANCE] (baudrate: [RATE])',
                    }),		    		    
                    blockType: BlockType.COMMAND,
                    arguments: {
                        INSTANCE: {
                            type: ArgumentType.STRING,
                            defaultValue: '$uart'
			},			
                        UART: {
                            type: ArgumentType.NUMBER,
                            defaultValue: 2
			},
                        RATE: {
                            type: ArgumentType.NUMBER,
                            defaultValue: 9600
			}			
                    }
                },
                {
                    opcode: 'uart_puts',
                    text: formatMessage({
                        id: 'unifiedapi.uart_puts',
                        default: 'UART: [INSTANCE] send string [COMM]',
                    }),		    		    
                    blockType: BlockType.COMMAND,
                    arguments: {
                        INSTANCE: {
                            type: ArgumentType.STRING,
                            defaultValue: '$uart'
			},			
                        COMM: {
                            type: ArgumentType.STRING,
                            defaultValue: "Output String"
                        }
                    }
                },
                {
                    opcode: 'uart_gets',
                    text: formatMessage({
                        id: 'unifiedapi.uart_gets',
                        default: 'UART: [INSTANCE] read string',
                    }),		    		    
                    blockType: BlockType.REPORTER,
                    arguments: {
                        INSTANCE: {
                            type: ArgumentType.STRING,
                            defaultValue: '$uart'
			}
                    }
                },		
                {
                    opcode: 'uart_clear',
                    text: formatMessage({
                        id: 'unifiedapi.uart_clear',
                        default: 'UART: [INSTANCE] clear [TXRX] buffer',
                    }),		    		    
                    blockType: BlockType.COMMAND,
                    arguments: {
                        INSTANCE: {
                            type: ArgumentType.STRING,
                            defaultValue: '$uart'
			},
                        TXRX: {
                            type: ArgumentType.STRING,
                            menu: 'menuUARTbuffer'
			}
                    }
                },		
                {
                    opcode: 'num16',
                    text: formatMessage({
                        id: 'unifiedapi.num16',
                        default: ' 0x[NUM]'
                    }),
                    blockType: BlockType.REPORTER,
                    arguments: {
			NUM: {
                            type: ArgumentType.STRING,
                            defaultValue: '77'
                        }
                    }
                },
                {
                    opcode: 'tools',
                    text: formatMessage({
                        id: 'unifiedapi.tools',
                        default: '[STR][TOOL]'
                    }),
                    blockType: BlockType.REPORTER,
                    arguments: {
			STR: {
                            type: ArgumentType.STRING,
                            defaultValue: 'string'
                        },
			TOOL: {
                            type: ArgumentType.STRING,
                            menu: 'menuTools'
                        }
                    }
                },
	    ],
            menus: {
                menuGPIOdirection:  { acceptReporters: false, items: createMenuItems('menuGPIOdirection') },
                menuUARTbuffer:  { acceptReporters: false, items: createMenuItems('menuUARTbuffer') },
                menuTools:  { acceptReporters: false, items: createMenuItems('menuTools') }
            }    
        };
    }

    // ブロックがクリックされた時の挙動．何もしない．   
    gpio_init(){}
    gpio_output_init () {}
    gpio_write () {}
    gpio_input_init () {}
    gpio_read () {}
    pwm_init () {} 
    pwm_duty () {}
    pwm_frequency () {}
    pwm_pulse () {}
    adc_init () {}
    adc_raw () {}    
    adc_volt () {}    
    i2c_init () {}
    i2c_write () {}
    i2c_read () {}
    uart_init () {}
    uart_puts () {}    
    uart_gets () {}    
    uart_clear () {}    
    spi_init () {}
    spi_write () {}
    spi_read () {}
    num16 () {}    
    tools () {}
    puts () {}
    p () {}
}

module.exports = UnifiedAPI
